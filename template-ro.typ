// ============================================================
//  sa-template-duo.typ  –  Solution Architecture document template
//  Rijksoverheid / DUO variant (logo embedded, no external files needed):
//    • no accent bar on cover
//    • Rijksoverheid logo top-centre (13 × 26 mm)
//
//  Usage:
//    md2typst input.md output.pdf --template sa-template-duo.typ
//
...
// ── Accent colour ────────────────────────────────────────────
#let accent = rgb("#1a6bbf")


// ── Embedded logo (Rijksoverheid) ───────────────────────
...
#let template(fm: (:), doc) = {
  ...
  set page(
    ...
    header: context {
      // Page 1 = cover, page 2 = TOC (both suppress via explicit page());
      // show header from page 3 onwards (first content page).
      if counter(page).get().first() > 2 {
        set text(size: 8pt, fill: rgb("#888888"))
        grid(
          columns: (1fr, 1fr),
          align(left)[#title],
          align(right)[versie #version],
        )
  ...
  // ── Typography ───────────────────────────────────────────────
  set text(font: ("Verdana", "Arial", "Helvetica Neue", "Liberation Sans"), size: 10.5pt, lang: _lang)
  ...
  // ── Cover page ───────────────────────────────────────────────
  page(
    margin: (top: 0pt, bottom: 2.5cm, left: 2.8cm, right: 2.5cm),
    header: none,
    footer: none,
  )[
    // Rijksoverheid logo — 13 × 26 mm, centred, flush to the top of the paper
    #align(center)[
      #image(bytes(_logo-svg), format: "svg", width: 13mm, height: 26mm, fit: "contain")
    ]
    #v(1.5cm)
